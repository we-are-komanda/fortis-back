...
func domainToImportResponse(p *domain.DefenseProject) ImportResponse {
	...
	resp.Body.ProjectVersion = p.Version()
	resp.Body.SnapshotDigest = p.SnapshotDigest()
	resp.Body.Snapshot = json.RawMessage(p.Snapshot())
...
func domainToProjectResponse(p *domain.DefenseProject) ProjectResponse {
	return ProjectResponse{
		ProjectVersion: p.Version(), SnapshotDigest: p.SnapshotDigest(), Snapshot: json.RawMessage(p.Snapshot()),
