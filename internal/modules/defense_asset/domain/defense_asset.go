...
var ValidDefenseAssetCategories = []DefenseAssetCategory{
	"early-warning", "detection", "classification", "jamming", "spoofing", "kinetic", "interceptor", "passive-protection", "engineering-protection", "software", "command-center", "external-service",
...
var ValidDefenseAssetRoles = []DefenseAssetRole{
	"detect", "track", "classify", "suppress", "destroy", "delay", "protect", "coordinate", "monitor", "alert",
...
// Validate checks the same complete aggregate before creation and update.
func (a *DefenseAsset) Validate() error {
	if a.name == "" {
		return ErrDefenseAssetInvalidName
	}
	if !isValidCategory(a.category) {
		return fmt.Errorf("%w: %s", ErrDefenseAssetInvalidCategory, a.category)
	}
	if !isValidCoverageType(a.coverageType) {
		return fmt.Errorf("%w: %s", ErrDefenseAssetInvalidCoverageType, a.coverageType)
	}
	if !isValidDeploymentType(a.deploymentType) {
		return fmt.Errorf("%w: %s", ErrDefenseAssetInvalidDeploymentType, a.deploymentType)
	}
	if !isValidPlacementType(a.placementType) {
		return fmt.Errorf("%w: %s", ErrDefenseAssetInvalidPlacementType, a.placementType)
	}
	return validateCategorySpecCompatibility(a.category, a.weaponSpec, a.detectionSpec, a.ewSpec)
}
...
func (a *DefenseAsset) SetPricePerUnitMln(v *float64) {
	a.pricePerUnitMln = v
...
func (a *DefenseAsset) SetCompatibleLayerTypes(v []LayerType) {
	a.compatibleLayerTypes = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetRecommendedLayerCodes(v []string) {
	a.recommendedLayerCodes = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetCompatibleLayerCodes(v []string) {
	a.compatibleLayerCodes = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetIncompatibleLayerCodes(v []string) {
	a.incompatibleLayerCodes = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetProtectionType(v string) {
	a.protectionType = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetMinEffectiveDistance(v *float64) {
	a.minEffectiveDistance = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetMaxEffectiveDistance(v *float64) {
	a.maxEffectiveDistance = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetCoverageRadius(v *float64) {
	a.coverageRadius = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetCoverageAngle(v *float64) {
	a.coverageAngle = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetDeploymentType(v DeploymentType) {
	a.deploymentType = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetPlacementType(v PlacementType) {
	a.placementType = v
	a.updatedAt = time.Now().UTC()
}
...
func (a *DefenseAsset) SetPriority(v *DefensePriority) {
	a.priority = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetCompoundProfile(v *DefenseAssetCompoundProfile) {
	a.compoundProfile = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetWeaponSpec(v *WeaponSpecification) {
	a.weaponSpec = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetDetectionSpec(v *DetectionSpecification) {
	a.detectionSpec = v
	a.updatedAt = time.Now().UTC()
}
...
func (a *DefenseAsset) SetCalculatorAssetID(v *string) {
	a.calculatorAssetID = v
	a.updatedAt = time.Now().UTC()
}
func (a *DefenseAsset) SetMapCatalogGroupIDs(v []string) {
	a.mapCatalogGroupIDs = v
	a.updatedAt = time.Now().UTC()
}
